 ...
 * the hint instead.
 *
 * idx and paddr both advance by block_size per entry, so alignment of
 * either is invariant across a run of entries within this call: once a
 * window at some idx/paddr qualifies (or fails to qualify) for the hint,
 * every later whole window does too, up to the end of num_entries. That
 * lets a run of qualifying (or non-qualifying) windows be merged into a
 * single arm_lpae_init_pte() call instead of one call per num_cont window.
 */
				 ...
				 arm_lpae_iopte *ptep, size_t *mapped)
{
	...
	while (done < num_entries) {
		...
		if (off) {
			...
			/*
			 * Aligned idx but paddr doesn't line up with
			 * cont_size, or too short for a full group - that
			 * holds for the rest of this call too, so install
			 * the remainder plain in one go.
			 */
			chunk = remaining;
		}
